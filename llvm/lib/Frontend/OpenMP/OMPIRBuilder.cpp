...
static Function *createOutlinedFunction(
  ...
  FunctionType* FuncType = FunctionType::get(Builder.getVoidTy(), ParameterTypes,
                                    /*isVarArg*/ false);
  Function* Func = Function::Create(FuncType, GlobalValue::InternalLinkage, FuncName,
  ...
  for (auto InArg : zip(Func->args(), Inputs)) {
    ...
    // dig a little to find an instruction so we can tell if they're used inside of the 
    // function we're outlining. We also replace the original constant expression 
    // with a new instruction equivelant; an instruction as it allows easy modification
    // in the following loop, as we can now know the constant (instruction) is owned by 
    // our target function and replaceUsesOfWith can now be invoked on it (cannot do this 
    // with constants it seems), a brand new one also allows us to be cautious as it is 
    // perhaps possible the old expression was used inside of the function but exists 
    // and is used externally (unlikely by the nature of a Constant, but still)
...
OpenMPIRBuilder::InsertPointTy OpenMPIRBuilder::createTarget(
  ...
  emitTargetOutlinedFunction(*this, Builder, EntryInfo, OutlinedFn,
                             ...
                             ArgAccessorFuncCB, CBFunc, AllocaIP);
  if (!Config.isTargetDevice())
...
void OpenMPIRBuilder::setCorrectMemberOfFlag(
    omp::OpenMPOffloadMappingFlags &Flags, omp::OpenMPOffloadMappingFlags MemberOfFlag) {
